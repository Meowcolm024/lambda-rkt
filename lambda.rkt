...
(define (handle-term expr) 
    (if (lambda? (car expr))
        (let ((lam (eval-expr (car expr)))
             ...
             (let ((left-vars (find-vars (caddr lam)))
                   (right-vars (find-vars body)))
                  (let ((mid (handle-dup (cadr lam) left-vars)))
                    (replace 
                        (replace-vars (caddr lam) left-vars mid) (car (cadr lam)) 
                        (replace-vars body right-vars (handle-dup mid (find-vars body)))))))
